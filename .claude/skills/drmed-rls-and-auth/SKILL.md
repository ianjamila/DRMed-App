...
**Since patient delete/restore (0167), `current_patient_id()` is SECURITY DEFINER and ACTIVE-ONLY.** It still reads the GUC or the JWT claim, but now also requires `deleted_at is null and merged_into_id is null` on the matching `patients` row — a signed session cookie for a since-deleted or since-merged record resolves to nothing on the very next query, whatever time is left on the cookie or the JWT. `getActivePatientSession()` (`src/lib/auth/require-patient.ts`) is **the portal's only session check**: it re-derives the bare cookie session (`getPatientSession()`) and then re-reads the patient row through `activePatients(...)`, returning `null` on a deleted/merged/missing record. `requirePatientProfile()` calls it and no longer follows the old merge chain. `src/lib/portal/active-session.test.ts` scans **all of `src/`**, not just the portal folder, and fails if anything outside `src/lib/auth/{patient-session-cookies,require-patient}.ts` calls the bare `getPatientSession()` directly — a new portal entry point (or any unrelated file) must go through `getActivePatientSession()`/`requirePatientProfile()`, never the raw cookie helper. Every caller of `getActivePatientSession()` answers a `null` with the same generic "Session expired. Sign in again." wording — never the record's lifecycle state (don't leak "this record was deleted" to whoever is holding the cookie).

**A Server Component render can't clear a cookie**, so a signed-but-now-inactive cookie can't be cleared from inside `requirePatientProfile()` itself — it redirects to `/portal/login/clear-session` instead of straight to `/portal/login`. That route (`src/app/(patient)/portal/login/clear-session/route.ts`) re-derives both facts itself rather than trusting why the browser landed there (hardened after a review that flagged the earlier version clearing unconditionally, which a bare cross-site link to the URL could have abused): no signed cookie → `/portal/login`; signed cookie + still active → leave it alone, `/portal`; signed cookie + inactive → clear it, then `/portal/login`.
...
**The two public surfaces share one module:** `src/lib/consent/self-registration.ts`. `selfRegistrationGrant({patientId, ip, userAgent})` builds the row (method `self_registration`, `actor_kind:'patient'`, no `created_by`, stamped with `CURRENT_CONSENT_NOTICE_VERSION`) and both `/register` and `/schedule` insert it via the admin client. The rule on both is the same — record consent **only** when this submission CREATED the patient and the required box was ticked; a dedup match, an existing patient or a portal booking NEVER gets its consent re-affirmed from a public form, because a public form must not write consent for a patient it merely recognised.

**Both write it through the same helper** — `recordSelfRegistrationGrant(deps, input)` in that module (added by the September 2026 consent-write fix). It inserts the row, and on failure calls the injected reporter (each caller binds `reportError` with its own scope, `register/consent-grant` or `schedule/consent-grant`) and returns `false` instead of throwing. **Never fail the submission over a failed consent write** — the patient row already exists, so the caller records the truth in its audit row (`consent_recorded`) and the patient surfaces under Admin > Patients without consent for capture at the counter (`patients.consent_current` was never flipped). Deps are injected so the helper stays unit-tested without a DB (`src/lib/consent/self-registration.test.ts`); use it for any new public channel rather than hand-rolling the insert.

They still reach the *gate* differently, which matters when you change either:
- `/schedule` calls the shared gate `shouldRecordBookingConsent(resolution, agreed)` and passes `booking_group_id` as extra failure metadata; the `appointment.booked` audit row carries `consent_recorded`.
- `/register` gates inline instead — early `return`s on the fuzzy-dedup match and on `res.reused`, then an explicit re-check of `data_privacy_consent` — and its `patient.self_registered` audit row carries `consent_recorded`.
...
- **Portal gate — every entry point, not just the layout.** `portalConsentCurrent(patientId)` (`src/lib/portal/consent-guard.ts`, React-`cache`d, follows a merge) is checked by the (authenticated) layout AND by every page, route handler and Server Action under it, before any read or audit row: actions/routes never run the layout, and a layout neither re-renders on client navigation nor runs before its page (they render in parallel). A page renders `<PortalConsentGate />`, an action returns `PORTAL_CONSENT_REQUIRED_ERROR`, the data export redirects to `/portal`. The shared booking action (`(marketing)/schedule/actions.ts`) checks it on its `source=portal` branch; the anonymous `/schedule` path records its own consent and stays open. Owner decision 2026-09-25: withdrawal blocks record access too (front desk prints instead). Exempt by design: `deletePatientLabRequestUpload` (erasure) and `logPatientStatementPrintAction` (audits a print that already happened). `consent-guard.test.ts` fails on a new entry point without it.
...
Buckets are declared in `RATE_LIMITS` in `check.ts` and their callers: `staff_login` (`/staff/login`), `patient_pin` (`/portal/login`), `patient_lookup` (existing-patient lookup on `/schedule` and `/portal/login`), `patient_id_recovery` (`/find-my-id`), `patient_registration` (`/register`), `public_booking` / `portal_booking` (`/schedule` anonymous vs signed-in portal `/book`), `appointment_cancel` (`/appointments/cancel/[id]`), `contact_form`, `newsletter_signup` / `newsletter_resubscribe` (`/newsletter`, `/unsubscribe`). A new public form gets its own bucket name. **Public forms insert with the service-role client and give anon NO table access** — 0154 removed `contact_messages`' anon `with check (true)` INSERT policy, which let anyone holding the public anon key skip the /contact honeypot and rate limit by writing through PostgREST. The Website Messages alert email to staff deliberately carries no message text, phone or email address (first name, subject, corporate flag and a sign-in link only); its audit row logs recipient counts, never addresses.
...
- **Reception and result PDFs (2026-09-24):** reception may open a result PDF only once the line is `released`, not a doctor kind, and every test on that shared file is released — `canViewResultPdf` in `src/lib/visits/line-visibility.ts`, enforced by `/staff/results/[id]/pdf` (service-role storage read, so the app gate IS the gate). `?print=1` audits `result.printed_staff` instead of `result.viewed_staff`. The lab queue lifts reception's `[]` section deny on its `Released today` tab ONLY, behind a redirect that keeps reception off every other tab. No RLS change: `results` / `result_test_requests` were already readable by every staff role (0151 "authenticated read"). **Print all (2026-09-25):** `/staff/visits/[id]/results-pdf` merges the visit's printable released files (`printAllFiles`, same rule) with pdf-lib and writes ONE audit row per file (`resource_id` = a line on it, `metadata.result_id`, `metadata.amendment_count`, `metadata.test_request_ids`, `metadata.bundle`); no `ignoreEncryption` (pdf-lib cannot decrypt, so an encrypted upload returns "print them one by one"). **"Printed … by …" notes:** `fetchPrintSummaries` (`src/lib/results/print-history.ts`, server-only) reads `result.printed_staff` rows through the service-role client — audit_log is admin-only under RLS — keyed by `metadata->>result_id`, and returns only a count, time and staff name (the `countResultViews` precedent). Every print writer stamps `metadata.result_id` AND `metadata.amendment_count` (the file version) — the note counts only prints of the current version, so an amended PDF (same result id, bumped count) reads as not yet handed over; a writer that swaps a file without bumping `amendment_count` would carry old prints over. The read pages with `fetchCompleteRows` (no silent 1000-row cap).
...
- **An export the BROWSER builds audits through a Server Action instead.** The two HMO controls on `admin/accounting/hmo-claims` (`ExportCsvButton` → a `Blob` download) exist so the file is exactly the rows currently on screen, filters and all; there is no request for `reportCsvResponse` to hang a row on. They call `recordHmoExportAuditAction`, backed by `recordHmoExportAudit` in `src/lib/reports/export-audit.ts`, which shares one metadata builder with `reportCsvResponse` so the two row shapes cannot drift. **Prefer a Route Handler**: this path only exists where "exactly what is on screen" is the requirement. Its cost is that `rows_exported` and the filters are asserted by the client — attribution (actor, time, IP/UA) stays server-proven, and the action re-proves its payload against a zod enum because every export of a `"use server"` file is a callable endpoint. **The download must fire synchronously with the click and the audit follow it** — WebKit ties `<a download>` to the user activation, and an awaited round-trip in between can silently eat it. An empty filtered set writes no row.
- **Any print or view that discloses patient data is audit-logged** even though it's a read: `result.viewed`, `result.downloaded`, `lab_request.viewed`, `payroll_payslip.viewed`, `eod_close.count_sheet_viewed`, `pf_disbursement.slip_printed`, `patient.data_exported`, `visits.exported`, `emails_log.exported`, `consent.artifact_viewed`, and the nine `report.<key>.exported` rows (`report.daily_revenue.exported`, `report.deleted_entries.exported`, `report.hmo_aging.exported`, `report.hmo_unbilled.exported`, `report.lab_tat.exported`, `report.patients_without_consent.exported`, `report.staff_advances.exported`, `report.stuck_tests.exported`, `report.undone_releases.exported`). Follow the same shape (actor, resource, IP/UA, metadata that names what was shown — never the content).
