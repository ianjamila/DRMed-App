...
- **The visit Statement of account (`…/visits/[id]/statement`) shares the receipt's A5 `receipt` page** (`.receipt-print` / `.receipt-sheet`) rather than adding its own — same size, same header, same `ReceiptLinesTable` (with `totalLabel="Total charges"`). It is the PIN-free paper (reception/admin only, audits `statement.viewed` / `statement.printed`); its paid figure is summed from the non-voided payments it lists (`statementSummary` in `src/lib/visits/statement.ts`), never read from `visits.paid_php`, so the page adds up on its own face. **One loader, two outputs:** `fetchStatement` (`src/lib/visits/statement-data.ts`, RLS-scoped client) feeds both the page and **Email to patient** (`…/statement/email-action.ts` → `renderStatementEmail` in `src/lib/visits/statement-email.ts`, pure + tested) — change what a statement shows there, not in the page. The email only goes to `patients.email` (no free-text recipient), is confirmed inline first, refuses a repeat within 2 minutes (`hasRecentAudit` on `statement.emailed`), and audits `statement.emailed` / `statement.email_failed` with `to`. `STATEMENT_ROLES` lives in `statement.ts`. Admin reaches it from Patient AR aging's Statement column too. Email tables use `emailAmountTable` in `src/lib/notifications/branded-email.ts`.
