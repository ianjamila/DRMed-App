...
0160_payment_correction.sql              ← "Edit payment": `payments.corrects_payment_id` (+ partial unique index) and service_role-only `correct_payment(...)` — insert the corrected payment, then void the original, one transaction; reference/notes-only edits update in place. P0054. Rules mirrored + pinned by `src/lib/visits/payment-edit.test.ts`. (0159 = `retire_send_out_accrual`, #211 — taken the same afternoon from a parallel branch.)
