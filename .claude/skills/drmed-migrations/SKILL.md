...
0154_website_messages_inbox.sql          ← drops the never-used `inquiries` (refuses if rows appear); `contact_messages` becomes the Website Messages inbox (status/kind/staff_notes/linked_appointment_id/attribution; P0053 immutability trigger; anon access REVOKED — 0004’s `with check (true)` anon INSERT policy let anyone skip the form’s honeypot + rate limit; reception/admin SELECT + UPDATE, no JWT DELETE); `appointments.source` + `attribution` backfilled from `appointment.booked` audit rows; `appointments_insert_slot_guarded` re-created to insert both (ACL restated service_role-only). CHECK lists pinned by `website-messages-schema.test.ts`; seed.sql carries the matching re-revoke.
