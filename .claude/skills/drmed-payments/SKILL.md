...
| Payment Changes report (admin) | `/staff/admin/reports/payment-changes` + `/api/admin/reports/payment-changes.csv` (audited `report.payment_changes.exported`); `src/lib/reports/payment-changes.ts`. Reads `payments` by `voided_at` window for deletes / money edits / moves, PLUS `audit_log` `payment.edited` with `metadata->>money_changed = 'false'` for reference/notes-only edits (those leave no voided row; `deriveInPlaceEdits`, entry id `audit-<id>`) |
