...
0161_payment_correction.sql              ← "Edit payment": `payments.corrects_payment_id` (+ partial unique index) and service_role-only `correct_payment(...)` — insert the corrected payment, then void the original, one transaction; reference/notes-only edits update in place. P0054. Rules mirrored + pinned by `src/lib/visits/payment-edit.test.ts`. (0159 = `retire_send_out_accrual`, #211; 0160 = `queue_claim_remarks`, #214 — both taken the same afternoon from parallel branches, so this was renumbered twice.)
0158_resolve_patient_referral_source.sql ← `resolve_patient_guarded` re-created (same signature, lock and service_role-only ACL, restated) to insert `referral_source` from `p_fields` on a NEW patient — through a `referral_sources` lookup so an unknown id is NULL, not an FK error — and still never write a matched row. Feeds "How did you hear about us?" on /schedule + /register. Pinned by `resolve-referral-migration.test.ts`.
