...
- **A printed `<tfoot>` repeats at the foot of every page**, so a multi-page bill shows the grand total under a partial list. Totals print as `print:table-row-group`. Both receipts render their lines through the shared `src/components/staff/receipt-lines-table.tsx` (package components sit under their header with blank amounts via `arrangeReceiptRows` in `src/lib/visits/receipt-totals.ts`) — change the table there, not per page.
