...
0175_patient_billed_catalog_read.sql    ← SELECT-only anon policies on `services` / `hmo_providers` / `discount_types` ("… patient billed"): a portal patient reads a catalog row only when one of their OWN visits references it (retired service on an old bill; statutory discount lookup). No claim → nothing new. For the portal statement of account (#212); proven both ways by `smoke:print`'s `portal-rls-direct`.
