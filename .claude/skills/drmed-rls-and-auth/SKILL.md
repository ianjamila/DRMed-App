...
They reach that rule differently, which matters when you change either:
- `/schedule` calls the shared gate `shouldRecordBookingConsent(resolution, agreed)` and **reports** a failed insert via `reportError` without rolling back the booking (the appointment exists; the `appointment.booked` audit row carries `consent_recorded` so staff can capture consent at the counter).
- `/register` gates inline instead — early `return`s on the fuzzy-dedup match and on `res.reused` — and its `insert` is **not** error-checked at all, so a failed consent write there is silent. Prefer the shared helper for a new channel, and treat `/register`'s unchecked insert as a known rough edge rather than the pattern to copy.
