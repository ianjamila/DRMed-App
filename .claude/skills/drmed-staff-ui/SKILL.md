...
| Print buttons (client `window.print()` wrappers) | `…/visits/[id]/receipt/print-button.tsx`, `…/visits/[id]/statement/print-button.tsx`, `…/payments/eod/[closeId]/count-sheet/print-button.tsx`, `…/admin/accounting/pf-payouts/[id]/slip/slip-print-button.tsx` |
...
- **The visit Statement of account (`…/visits/[id]/statement`) shares the receipt's A5 `receipt` page** (`.receipt-print` / `.receipt-sheet`) rather than adding its own — same size, same header, same `ReceiptLinesTable` (with `totalLabel="Total charges"`). It is the PIN-free paper (reception/admin only, audits `statement.viewed` / `statement.printed`); a waived visit (`payment_status = 'waived'`) shows the remainder as **Balance waived** and **Nothing due** — waiving writes no payment row; its paid figure is summed from the non-voided payments it lists (`statementSummary` in `src/lib/visits/statement.ts`), never read from `visits.paid_php`, so the page adds up on its own face. **One loader, one sheet, three outputs:** `fetchStatement` (`src/lib/visits/statement-data.ts`, takes the caller's client) feeds the staff page, the patient portal page (`/portal/visits/[id]/statement`, patient client) and **Email to patient** (`src/lib/actions/visits/email-statement.ts`, and the patient's portal **Email it to me** `…/portal/…/statement/email-action.ts` — both thin wrappers over `sendStatementEmail` in `src/lib/visits/send-statement-email.ts`, which owns the claim, send, release and audit; `requestedBy` changes only the footer line → `renderStatementEmail` in `src/lib/visits/statement-email.ts`, pure + tested). Both pages render `StatementSheet` (`src/components/statement/statement-sheet.tsx`) — change what a statement shows there and in the email renderer, never per page. The button is `src/components/staff/email-statement-button.tsx` (`size="compact"` + `accessibleName` for table rows — used in the patient page's Visits › Statement column). The email only goes to `patients.email` (no free-text recipient), is confirmed inline first, refuses a repeat within 2 minutes via `claim_statement_email` (0177: advisory-locked, keyed visit + recipient not actor, admits exactly one of N simultaneous senders, refused callers write nothing; the action deletes ITS OWN claim id when nothing went out — never a rival's), and audits `statement.emailed` / `statement.email_failed` with `to`. `STATEMENT_ROLES` lives in `statement.ts`. Admin reaches it from Patient AR aging's Statement column too. The Audit Log's quick-filter chips come from `AUDIT_PRESETS` (`src/lib/audit/presets.ts`); `presets.test.ts` fails if a preset's action prefix matches nothing the code writes. Email tables use `emailAmountTable` in `src/lib/notifications/branded-email.ts`.
...
- **The shell must not wrap a print surface in a scroll box or an in-flow widget.** `StaffShell`'s `<main>` scrolls wide tables on screen (`overflow-x-auto`) and is `print:overflow-visible` — printed, a scroll box is clipped with its scrollbar drawn in on any Mac showing scroll bars, which cut the right edge and the whole portal-access block off receipts (2026-09-24). The dashboard layout wraps sonner's `<Toaster>` in `print:hidden` because its empty live region alone pushed a blank last page. Verify a print change with real Chrome, not the headless shell: `chromium.launch({ channel: "chrome", ignoreDefaultArgs: ["--hide-scrollbars"] })` + `page.pdf({ preferCSSPageSize: true })` — the default headless shell hides scrollbars and never reproduces the clip. That recipe is packaged as **`npm run smoke:print`** (`scripts/smoke-print.ts`): against the LOCAL stack it seeds a throwaway patient/package visit/group/payment/payout/cash close, signs a temp admin in through the real login form, checks each print page's print-media DOM (no scroll box or clip around the sheet, no repeating `<tfoot>`, the text the paper must carry), prints to PDF with the page's own `@page` size and checks the page count, then clicks through the statement's two other doors (Email to patient → one send audit row to the seeded `.invalid` address; the Patient AR Statement link) then signs in to the portal as the seeded patient and prints their own statement (plus: view audited, another patient's visit 404s), and deletes everything it made (journal entries, PIN, consent and patient-actor audit rows included) even on failure. The dev server needs `SUPABASE_JWT_SECRET` from `supabase status -o env` for the portal steps. Scope a `getByRole("alert")` to your component — Next.js keeps an always-present `role="alert"` route announcer that matches instantly. Every request carries a per-run `x-forwarded-for` (IPv6 documentation range) so teardown deletes only this run's login-throttle rows. Seeded visits need `visits.total_php` set by hand (the app stamps it; no trigger derives it). The local stack is shared: another session's `supabase db reset` kills a run mid-way (`Connection terminated unexpectedly`) — wait for quiet (`pgrep -f "supabase db reset"`) and rerun. Run a dev server (`npx next dev -p 3007`) then `APP_BASE=http://localhost:3007 npm run smoke:print`. **Add every new print surface to its surface list.** The three shell fixes are also pinned source-level in `src/lib/staff/print-shell.test.ts`.
