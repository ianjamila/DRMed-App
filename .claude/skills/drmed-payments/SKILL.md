...
| Payment Changes report (admin) | `/staff/admin/reports/payment-changes` + `/api/admin/reports/payment-changes.csv` (audited `report.payment_changes.exported`); `src/lib/reports/payment-changes.ts`. **Delete reason** column + `why` filter (a category, or `none` = deletes from before the picker) and CSV column; the Deleted tile's `deletedByReason` breakdown links to each filter. Reads `payments` by `voided_at` window for deletes / money edits / moves, PLUS `audit_log` `payment.edited` with `metadata->>money_changed = 'false'` for reference/notes-only edits (those leave no voided row; `deriveInPlaceEdits`, entry id `audit-<id>`) |
| Completed work but owing again (non-HMO) | Reachable ONLY by a payment deleted, edited down or moved after work was completed — there is no add-a-line path (the only `test_requests` insert is registration; a later test is a NEW visit under the normal gate). **"Completed work"** = released lab/imaging results + doctor consult/procedure lines marked done (`completedWorkCount` / `loadCompletedWorkCounts` in `released-results.ts`, an `all` surface; `countReleasedLines` keeps the results-vs-doctor wording split). Visit page amber note (`releasedWhileUnpaidMessage`) replaces "Releases are blocked"; Reception Queue Waiting rows get a **Completed work · N** badge (counted from the queue's own `test_requests` embed); Patient AR's non-HMO tab is **Exceptions (non-HMO)** (pay-first: these rows are corrections or refunds, not receivables — default scope and all positive balances kept) with the badge + an opt-in **Completed work** filter (URL key still `?released=1`, non-HMO only — HMO visits release unpaid by design). Delete, Edit (amount reduced) and Move all send `released_payment_removed` when `settled_after=false && completed_work>0`; audit rows carry `released_count` (results) + `completed_work`. **0125's header comment ("an unpaid visit can never have released tests") is FALSE** — the code (`deletion.ts`, P0043) is right to check the line status itself |
...
| Cash drawer / EOD | `/staff/payments/cash-drawer`, `/staff/payments/eod` (close by denomination; `CloseEodSchema` in `src/lib/validations/accounting.ts` — `counted_cash_php` is derived server-side, not posted), `/staff/payments/eod/[closeId]/count-sheet` (A5 print, reception/admin, audited `eod_close.count_sheet_viewed`), `/staff/payments/petty-cash`. EOD has a date/shift picker (`?date=`/`?shift=`, max today; client keyed on date:shift so drafts never carry over). **End of Day reminders (0185):** `accounting_settings.eod_reminders_start_date` (blank = off, set on Money Routing) + service_role-only `eod_unclosed_days(from, to, shift)` — days from the start date to yesterday that moved cash per `cash_drawer_state` and have no closed record; read via `src/lib/accounting/eod-reminders.ts` by Cash In & Out / EOD / the reception + admin dashboards (`src/components/staff/unclosed-days-notice.tsx`; dashboards via `_dashboards/_components/eod-reminder-banner.tsx`, first active shift, not hideable) and the Cash & cards report + CSV (`CashReconRow.notClosedShiftIds`) |
