...
└── 0147_ap_cash_bill_payment_drawer_link.sql ← the THIRD cash door: an AP bill paid out of 1010 now writes its own eod_cash_adjustments row (kind='bill_payment' + bill_payment_id, the gift-code biconditional shape) from an AFTER INSERT trigger on bill_payments — covering BOTH doors into that table, ap_create_bill_payment_with_allocations AND ap_create_bill_paid_on_entry. The row posts NO JE (bridge_cash_adjustment_insert early-returns; ap_bill_payment_bridge already credited 1010 — remove the early return and it is credited twice) and cannot be voided from the drawer (P0051); the void mirrors from bill_payments.voided_at. P0050 covers no-active-shift / no-recording-staff. cash_drawer_state re-created to count the kind as a payout. No backfill: all 75 prod rows are one 2026-05-28 import batch on days that were never closed or counted.
