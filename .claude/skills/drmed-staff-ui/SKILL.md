...
- **The visit Statement of account (`…/visits/[id]/statement`) shares the receipt's A5 `receipt` page** (`.receipt-print` / `.receipt-sheet`) rather than adding its own — same size, same header, same `ReceiptLinesTable` (with `totalLabel="Total charges"`). It is the PIN-free paper (reception/admin only, audits `statement.viewed` / `statement.printed`); its paid figure is summed from the non-voided payments it lists (`statementSummary` in `src/lib/visits/statement.ts`), never read from `visits.paid_php`, so the page adds up on its own face. **One loader, one sheet, three outputs:** `fetchStatement` (`src/lib/visits/statement-data.ts`, takes the caller's client) feeds the staff page, the patient portal page (`/portal/visits/[id]/statement`, patient client) and **Email to patient** (`src/lib/actions/visits/email-statement.ts` → `renderStatementEmail` in `src/lib/visits/statement-email.ts`, pure + tested). Both pages render `StatementSheet` (`src/components/statement/statement-sheet.tsx`) — change what a statement shows there and in the email renderer, never per page. The button is `src/components/staff/email-statement-button.tsx` (`size="compact"` + `accessibleName` for table rows — used in the patient page's Visits › Statement column). The email only goes to `patients.email` (no free-text recipient), is confirmed inline first, refuses a repeat within 2 minutes (`hasRecentAudit` on `statement.emailed`), and audits `statement.emailed` / `statement.email_failed` with `to`. `STATEMENT_ROLES` lives in `statement.ts`. Admin reaches it from Patient AR aging's Statement column too. The Audit Log's quick-filter chips come from `AUDIT_PRESETS` (`src/lib/audit/presets.ts`); `presets.test.ts` fails if a preset's action prefix matches nothing the code writes. Email tables use `emailAmountTable` in `src/lib/notifications/branded-email.ts`.
