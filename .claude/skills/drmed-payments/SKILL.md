...
| `src/lib/accounting/pg-errors.ts` — `translatePgError` | P0001–P0035, P0040–P0051 → user-facing strings. Add a case for every new `raise`; `pg-error-coverage.test.ts` (0147) now greps every migration and fails on an untranslated code. `translateBillPaymentError` wraps it for the two AP payment doors, which need P0015 said differently. |
