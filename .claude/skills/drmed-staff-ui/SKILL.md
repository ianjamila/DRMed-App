...
- Each tab set is a thin wrapper that just declares its `TABS` and renders `<SectionTabs/>` (see `bills-tabs.tsx`, `visits-tabs.tsx`, `statement-tabs.tsx`, `payments-tabs.tsx`). A wrapper that carries a selection must be a **client** component. `src/components/staff/section-tabs-query.test.tsx` pins the rendered hrefs for the three that do.
