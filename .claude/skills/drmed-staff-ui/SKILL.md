...
- **The visit Statement of account (`…/visits/[id]/statement`) shares the receipt's A5 `receipt` page** (`.receipt-print` / `.receipt-sheet`) rather than adding its own — same size, same header, same `ReceiptLinesTable` (with `totalLabel="Total charges"`). It is the PIN-free paper (reception/admin only, audits `statement.viewed` / `statement.printed`); its paid figure is summed from the non-voided payments it lists (`statementSummary` in `src/lib/visits/statement.ts`), never read from `visits.paid_php`, so the page adds up on its own face.
