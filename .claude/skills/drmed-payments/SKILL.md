...
| PF clawback on a void (DORMANT) | `bridge_payment_void_pf_cascade` (0064, `trg_bridge_payment_void_pf_cascade`) returns early while the visit still reads `paid` — and AFTER UPDATE triggers fire alphabetically, so it always runs BEFORE `trg_payments_recalc_on_void` and never sees a visit turn unpaid. It has never fired (0 cascade voids on prod; `doctor_pf_entries` empty 2026-09-25). Do NOT just reorder the triggers: the cascade reverses ALL PF on any drop below `paid`, never re-accrues when the visit is paid again, posts on UTC `current_date`, and its `auth.uid()` actor is NULL under the service-role client — so "delete the wrong payment, record the right one" would strip a doctor fee for good. PF-on-unpaid semantics are an owner decision |
