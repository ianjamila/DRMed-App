...
**"How did you hear about us?" (0157):** both public forms (`/schedule` new-patient mode — the About-you step — and `/register`) require a `referral_source` from `PUBLIC_REFERRAL_OPTIONS` (`src/lib/patients/referral-sources.ts`, the 12 `referral_sources` ids with patient wording; error "Tell us how you heard about us."). It rides in `p_fields` to `resolve_patient_guarded`, which since 0157 writes it on a NEW row only — looked up in `referral_sources`, so an unknown id becomes NULL rather than an FK error — and never touches a MATCHED row. Existing-patient and portal bookings are not asked. Both actions also put the answer in their audit metadata (`appointment.booked`, `patient.self_registered` / `.matched`) so a matched booking's answer is not lost. The staff slide-over does not ask (its bookings have `appointments.source` instead) and passes nothing → NULL. Booking Sources (`/staff/marketing/sources`) tallies new patients by it (`summarizeNewPatientReferrals`).
