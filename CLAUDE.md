...
| Manila/PHT date helpers (`todayManilaISODate`, `isISODate`, `shiftISODate`, `manilaRangeUtc`), the calendar arithmetic (`isoDateParts`, `firstOfMonthISO`, `lastOfMonthISO`, `daysInMonth`) + the canonical display formatters `manilaDate` / `manilaDateTime` / `manilaTime` / `manilaLongDate` / `friendlyManilaDate` — never build a date format in a page; `src/lib/dates/date-render-surfaces.test.ts` enforces it | `src/lib/dates/manila.ts` |
