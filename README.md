...
Every runner with a `--commit` mode needs a `--confirm` value that identifies
the database it is pointed at — `local` for the local stack, the Supabase
**project ref** for anything remote. The dry-run prints the exact token:
...
This applies to `wipe:operational`, `dedup:patients`, `import:legacy`,
`enrich:clinical`, `backfill:clinical:{lab,consult}`, the two
`clinical-backfill` follow-ups, and all four `import:history:*` runners.

It replaced a fixed `--confirm="I-mean-it"` passphrase. A constant is the same
eleven characters on every target, so it becomes muscle memory; a token you can
only produce by reading the guard banner is a statement about *which* database
you are about to change. A prod token does not work against local, or the
reverse, and a run whose REST endpoint and `SUPABASE_DB_URL` name different
projects is refused outright rather than counting rows in one database and
truncating another.

Reach for `requireTargetConfirmation()` from `scripts/lib/env-guard.ts` when
adding another one — do not invent a second confirm flag.
