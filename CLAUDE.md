...
- `docs/drmed-user-guide.html` — the staff + patient user guide (v2.2, 14 Sep 2026): every
...
Migration ledger: **prod head = 0145**, repo↔prod in sync (2026-09-14). There is ONE
...
| Manila/PHT date helpers (`todayManilaISODate`, `isISODate`, `shiftISODate`, `manilaRangeUtc`) + the canonical display formatters `manilaDate` / `manilaDateTime` / `manilaTime` / `manilaLongDate` / `friendlyManilaDate` — never build a date format in a page; `src/lib/dates/date-render-surfaces.test.ts` enforces it | `src/lib/dates/manila.ts` |
...
- **Soft delete:** filter `deleted_at is null` on `visits` / `test_requests` in every read surface (queues, results, dashboards, receipts, exports, portal, sheet export). Deletability is `payment_status = 'unpaid'` only. **Deleting a VISIT does not cascade to its `test_requests`** — 0125's only cascade is package header → components — so a `test_requests` read that means "live" needs BOTH `.is("deleted_at", null)` and `.is("visits.deleted_at", null)` over a `visits!inner` embed. `src/lib/visits/query-surfaces.test.ts` (the `LIFECYCLES` map) fails on any file that reads either table without declaring whether it means live rows or spans deleted ones. It scans TypeScript only: **ten SQL views read these tables with no `deleted_at` predicate** and are outside its reach. Nine of the ten need one — the four `v_hmo_*`, `v_daily_revenue_by_service`, and four of the five `v_ops_daily_*` all drive straight off `test_requests` on nothing but `status = 'released'`. Only `v_ops_daily_collections` is driven by `payments` (which P0045 keeps clean of deleted visits), and even it classifies its section with an unfiltered `exists` over `test_requests`. `v_hmo_unbilled` is the worst of them, since it feeds the HMO claim picklist. **`create or replace view` silently drops `security_invoker`** — restate it in every redefinition, and register the four unguarded `v_ops_daily_*` views in `src/lib/supabase/hardened-views.test.ts` when you touch them.
